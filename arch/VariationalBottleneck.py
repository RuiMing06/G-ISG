...
class VariationalBottleneck(torch.nn.Module):
    ...
    def __init__(self, in_shape, K=256, beta=1e-6, *args, **kwargs):
        super().__init__()
    ...
    def forward(self, x, eps):
        ...
        encoding = self.reparameterize(mu, std, eps)  # pull sample from the distribution
    ...
    def reparameterize(self, mu, std, eps):
        if eps is None:
            eps = torch.randn(1, 256).to(mu.device)
        ...
        return mu + eps.to(mu.device) * std

    def loss(self):
        return self.beta * (-0.5 * (1 + 2 * self.std.log() - self.mu.pow(2) - self.std.pow(2)).sum(1).mean().div(
            math.log(2)))  # KLD to standard normal distribution
