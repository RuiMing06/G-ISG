...
from piq import ssim,fsim,psnr,LPIPS
import time
...
def calculate_iqa(dataloader,reconstruct_data):
    score = {}

    ranged_data = []
    flag = [0] * len(dataloader)
    for i in range(len(dataloader)):
        mse_min = 1
        for j in range(len(reconstruct_data)):
            if flag[j] == 0:
                mse = torch.mean((reconstruct_data[j]-dataloader[i])**2).item()
                if mse < mse_min:
                    mse_min = mse
                    matched_data = reconstruct_data[j]
                    k = j
        ranged_data.append(matched_data)
        flag[k] = 1
    reconstruct_data = torch.stack(ranged_data)

    for metric in ['mse','ssim','psnr','lpips']: 
        if (metric == 'mse'):
            each_score = torch.mean((reconstruct_data-dataloader)**2).item()
        elif (metric == 'ssim'):
            each_score = ssim(dataloader,reconstruct_data).item()
        elif (metric == 'psnr'):
            each_score = psnr(dataloader,reconstruct_data).item()
        elif (metric == 'lpips'):
            lpips = LPIPS()
            each_score = lpips(dataloader,reconstruct_data).item()
        score[metric] = each_score
    return score

def save_images(imgs,arch,datasets):
    result_folder_name = f"./result/{datasets}/{arch}"
