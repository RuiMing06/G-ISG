...
def convert_relu_to_sigmoid(model):
    for child_name, child in model.named_children(): #遍历子模块迭代器（模块名称+模块本身）
        if isinstance(child, nn.ReLU):
            setattr(model, child_name, nn.Sigmoid()) #将model中的所有relu子模块设置为sigmoid函数
        else:
...
def loss_inverting_gt(original_dy_dx,fake_dy_dx,fake_img,tv_value):
    ...
    errG+= tv_value * total_variation(fake_img) #TV正则化项
    return errG
...
def loss_cosine_similarity(original_dy_dx,fake_dy_dx):
    errG = 0
    pnorm = [0,0]
    for gx,gy in zip(fake_dy_dx, original_dy_dx):
        errG -= (gx * gy).sum()
