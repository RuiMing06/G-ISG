...
class GradientReconstructor:
    ...
    def __init__(self, model, parameters, mean_std=(0.0, 1.0), config=DEFAULT_CONFIG, num_images=1):
        """Initialize with algorithm setup."""
        self.reconstruct_label = None
        self.config = config
        self.model = model
    ...
    def reconstruct(self, input_data, labels, img_shape=(3, 32, 32), dryrun=False, eval=True, tol=None):
        ...
        return x_optimal.detach(), labels.detach()
    ...
    def _run_trial(self, x_trial, input_data, labels, dryrun=False):
        ...
        if self.reconstruct_label:
            ...
            optim_obj = [x_trial, labels] + self.parameters['eps'] + self.parameters['multiplier']

            if self.config['optim'] == 'adam':
                optimizer = torch.optim.Adam(optim_obj, lr=self.config['lr'])
            elif self.config['optim'] == 'sgd':  # actually gd
                optimizer = torch.optim.SGD(optim_obj, lr=0.01, momentum=0.9, nesterov=True)
            elif self.config['optim'] == 'LBFGS':
                optimizer = torch.optim.LBFGS(optim_obj)
            else:
                raise ValueError()
        else:
            optim_obj = [x_trial, labels] + self.parameters['eps'] + self.parameters['multiplier']
            # optim_obj = [x_trial, labels] + self.parameters['eps']
    ...
    def _gradient_closure(self, optimizer, x_trial, input_gradient, label):
        ...
        def closure():
            ...
            output = self.model(x_trial)
            # output, x_cor, x_ext = self.model(x_trial)
            loss = self.loss_fn(output, label) + self.model.loss()
            # loss = self.loss_fn(self.model(x_trial), label)
            gradient = torch.autograd.grad(loss, self.parameters['models'], create_graph=True)
            ...
            if self.config['kl_value'] > 0:
                rec_loss += self.config['kl_value'] * KL(self.parameters['eps']).item()
    ...
    def _score_trial(self, x_trial, input_gradient, label):
        if self.config['scoring_choice'] == 'loss':
            ...
            x_trial.grad = None
            output = self.model(x_trial)
            # output,_,_ = self.model(x_trial)
            loss = self.loss_fn(output, label)
            gradient = torch.autograd.grad(loss, self.parameters['models'], create_graph=False)
