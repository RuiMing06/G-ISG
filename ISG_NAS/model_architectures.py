...
def set_random_seed(seed=233):
    torch.manual_seed(seed + 1) #设置随机数种子
...
#     'VGG16': [64,64, 'M', 128, 'M', 256, 256, 'M', 512, 512, 'M', 512, 512, 'M'],
...
class VGG(nn.Module):
    def __init__(self, vgg_name):
        ...
        if(vgg_name=='VGG11'):
            self.classifier = nn.Linear(627200, 10)
        elif(vgg_name=='VGG16'):
            print("SET VGG16")
            self.classifier = nn.Linear(819200, 10)

    def forward(self, x):
        out = self.features(x)
        out = out.view(out.size(0), -1)
#         print("OUTPUT SIZEEEEEEE",out.size())
    ...
    def _make_layers(self, cfg):
        ...
        for x in cfg:
            if x =='M':
                layers += [nn.MaxPool2d(kernel_size=2, stride=(1, 1))]
            else:
                layers += [nn.Conv2d(in_channels, x, kernel_size=2, padding=1, stride=(1, 1)),
                           act()]
                in_channels = x
        layers += [nn.AvgPool2d(kernel_size=1, stride=(1, 1))]
        return nn.Sequential(*layers)