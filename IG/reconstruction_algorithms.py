...
class GradientReconstructor():
    ...
    def _gradient_closure(self, optimizer, x_trial, input_gradient, label):
        ...
        def closure():
            ...
            loss = self.loss_fn(self.model(x_trial), label) + self.model.loss()
            gradient = torch.autograd.grad(loss, self.parameters, create_graph=True)
    ...
    def _score_trial(self, x_trial, input_gradient, label):
        if self.config['scoring_choice'] == 'loss':
            ...
            gradient = torch.autograd.grad(loss, self.parameters, create_graph=False)
